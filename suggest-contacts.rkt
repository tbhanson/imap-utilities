...
;; (e.g. "[Gmail]/Sent Mail", "Sent", "Sent Items") and extracts
;; the To/CC addresses — people YOU have written to.
;;
;; Options:
...
;; Find all digest files whose folder name contains "sent" (case-insensitive)
(define (find-sent-mail-digests)
    ...
    (if (directory-exists? dir)
        (let ([all-files
               (for/list ([f (directory-list dir #:build? #t)]
                 ...
                 f)])
          ;; Load each and keep only those with "sent" in the folder name
          (for/fold ([result '()])
                    ([f all-files])
...
(define (parse-args args)
  (let ([min-count 2]
    ...
    (let loop ([remaining (vector->list args)])
      (cond
        [(null? remaining) (values min-count save?)]
    ...
    (values min-count save?)))

;; ---- main ----

(define (main)
  (let-values ([(min-count save?) (parse-args (current-command-line-arguments))])

    (printf "Scanning for sent-mail digests...~n")
    (let ([sent-digests (find-sent-mail-digests)])

      (when (null? sent-digests)
        (printf "~nNo sent-mail digests found.~n")
        (printf "Fetch your sent mail first, e.g.:~n")
        (printf "  racket fetch.rkt \"my-gmail\" \"[Gmail]/Sent Mail\"~n")
        (printf "  racket fetch.rkt \"my-other\" \"Sent\"~n")
        (exit 0))
